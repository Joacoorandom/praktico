...
type DeliveryMethod = "retiro_colegio" | "envio_starken";
...
function buildWhatsappConfirmMessage(params: {
  ...
  if (params.delivery.method === "envio_starken") {
    lines.push("Entrega: envío (Starken)");
...
export default function CheckoutPage() {
  ...
  // Reglas:
  // - Efectivo (alumnos): solo retiro en colegio.
  // - Transferencia: envío (Starken).
  useEffect(() => {
    if (paymentMethod === "efectivo") {
      setDeliveryMethod("retiro_colegio");
      // Limpia estado de envío cuando no aplica.
      setShippingState("idle");
      setShippingOptions([]);
      setSelectedShippingId(null);
      setShippingError("");
    } else {
      setDeliveryMethod("envio_starken");
    }
  }, [paymentMethod]);

  const selectedShipping = useMemo(() => {
    if (deliveryMethod !== "envio_starken") return null;
  ...
  const shippingCost = useMemo(() => {
    if (deliveryMethod !== "envio_starken") return 0;
  ...
  async function quoteShipping() {
    ...
    setSelectedShippingId(null);
    try {
      const resp = await fetch("/api/shipping/starken", {
        method: "POST",
  ...
  async function submitOrder() {
    ...
    try {
      const payload = {
        ...
        delivery: {
          method: deliveryMethod,
          destinationComuna: deliveryMethod === "envio_starken" ? destinationComuna : undefined,
          shippingCost: deliveryMethod === "envio_starken" ? shippingCost : undefined,
          etaDays: deliveryMethod === "envio_starken" ? selectedShipping?.diasEntrega ?? null : null,
          starken:
            deliveryMethod === "envio_starken"
              ? {
      ...
      <div className="card product-card">
        {step === "datos" ? (
          ...
          <>
            ...
            <p className="muted" style={{ marginTop: 6 }}>
              Elige el método de pago. Luego confirmas y se envía el pedido al negocio.
            </p>

            <div className="panel" style={{ marginTop: 12 }}>
              <div style={{ fontWeight: 800, marginBottom: 8 }}>Método</div>
              ...
              <label style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 10 }}>
                <input
                  ...
                  style={{ width: 18, height: 18 }}
                />
                Efectivo (solo Scuola Italiana o Instituto Luis Campino, Chile)
              </label>
            ...
            {paymentMethod === "transferencia" ? (
              <div className="panel" style={{ marginTop: 12 }}>
                <div style={{ fontWeight: 800, marginBottom: 8 }}>Envío (Starken)</div>
                <div className="muted" style={{ marginBottom: 10 }}>
                  Para cotizar, indica la comuna de destino. El costo de envío se suma al total.
                </div>
