...
Notas:
  ...
  - Permite imágenes por URL (https://..., ej. R2) además de /public.
  - Para API HTTP (productos, categorías, destacados): ver carpeta admin-server/.
`.trim();
}
...
async function cmdAddInteractive() {
  ...
  const featured = await confirm({ message: "¿Destacado (aparece en panel Ofertas)?", default: false });
